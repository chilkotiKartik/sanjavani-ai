        ...
        const saved = await this.store.conversations.persistTurn({
          ...
          triage: events.triage
            ? {
                ...
                // Stored beside the decision so the clinician review queue never has
                // to open a conversation to show what the rules acted on.
                severities: events.triage.symptoms.map((s) => s.severity),
                ageGroup: events.triage.ageGroup,
                durationHours: events.triage.duration?.hours ?? null,
                language: events.triage.language,
              }
            : null,
