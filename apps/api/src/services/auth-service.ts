...
export class AuthService {
  ...
  async createAdminSession(): Promise<IssuedSession> {
    return this.createStaffSession('ADMIN');
  }

  /**
   * A session for someone with a privileged role.
   *
   * Each sign-in mints a fresh user row, exactly as the anonymous path does, so a
   * reviewer's submissions are attributable to *that sitting* without the service ever
   * holding a name, an email or a credential belonging to a real person. Attribution
   * of a clinical judgement is worth having; a staff directory inside a triage app is
   * not, and would be one more thing to leak.
   */
  async createStaffSession(role: Exclude<Role, 'USER'>): Promise<IssuedSession> {
    const user = await this.store.users.create({ anonymous: false, role });
    return this.issue(user.id, role, false);
  }
