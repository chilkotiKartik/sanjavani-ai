...
describe('clinician review', () => {
  ...
  /** A reviewer session, plus one real triage decision for them to work on. */
  async function reviewSetup(env: Record<string, string> = {}) {
    const ctx = await setup({ REVIEWER_ACCESS_KEY: REVIEWER_KEY, ...env });
    const user = await session(ctx.app);
    const { body: conversation } = await user.post('/v1/conversations').send({});
    // Two turns, so the engine gets past its follow-up question and files a decision.
    await user.post(`/v1/conversations/${conversation.id}/turns`).send({
      text: 'I have had a high fever and body ache for two days',
      inputMode: 'text',
      location: LOCATION,
    });
    await user.post(`/v1/conversations/${conversation.id}/turns`).send({ text: 'no', inputMode: 'text', location: LOCATION });

    const login = await request(ctx.app).post('/v1/auth/reviewer').send({ accessKey: REVIEWER_KEY });
    expect(login.status).toBe(201);
    expect(login.body.role).toBe('REVIEWER');
    const auth = { Authorization: `Bearer ${login.body.token}` };
    return { ...ctx, auth, conversationId: conversation.id as string };
  }

  it('is switched off, and invisible, without a reviewer key', async () => {
    const { app } = await setup();
    // 404 rather than 401: an unconfigured deployment should not disclose that a
    // clinical review console exists here at all.
    expect((await request(app).post('/v1/auth/reviewer').send({ accessKey: 'x' })).status).toBe(404);
    const user = await session(app);
    expect((await user.get('/v1/review/queue')).status).toBe(403);
  });

  it('refuses an ordinary session and a wrong key', async () => {
    const { app } = await setup({ REVIEWER_ACCESS_KEY: REVIEWER_KEY });
    expect((await request(app).post('/v1/auth/reviewer').send({ accessKey: 'wrong' })).status).toBe(401);
    const user = await session(app);
    expect((await user.get('/v1/review/queue')).status).toBe(403);
    expect((await user.get('/v1/review/stats')).status).toBe(403);
  });

  /*
   * The property this whole surface rests on. A reviewer is shown the structured facts
   * the rules acted on and nothing that could identify the person or reveal what they
   * typed — so this asserts on the shape of the payload rather than on any one field,
   * and would fail if a future column started riding along.
   */
  it('shows a reviewer the decision and nothing that identifies anyone', async () => {
    const { app, auth, conversationId } = await reviewSetup();
    const queue = await request(app).get('/v1/review/queue').set(auth);
    expect(queue.status).toBe(200);
    expect(queue.body.cases.length).toBeGreaterThan(0);

    const [first] = queue.body.cases;
    expect(Object.keys(first).sort()).toEqual(
      ['ageGroup', 'confidence', 'decidedAt', 'durationHours', 'emergency', 'emergencyCategory', 'id', 'language', 'rationale', 'source', 'specialty', 'symptoms', 'urgency'].sort(),
    );
    expect(first.symptoms.map((s: { code: string }) => s.code)).toContain('fever');

    const serialised = JSON.stringify(queue.body);
    expect(serialised).not.toContain(conversationId);
    expect(serialised).not.toContain('high fever and body ache');
    expect(serialised.toLowerCase()).not.toContain('ciphertext');
  });

  it('records a verdict, refuses a second from the same reviewer, and counts it', async () => {
    const { app, auth } = await reviewSetup();
    const { body: queue } = await request(app).get('/v1/review/queue').set(auth);
    const caseId = queue.cases[0].id;

    const first = await request(app).post(`/v1/review/${caseId}`).set(auth).send({ verdict: 'AGREE' });
    expect(first.status).toBe(201);

    // A second opinion from the same person is not a correction, so it is refused
    // rather than allowed to overwrite the judgement already given.
    const again = await request(app).post(`/v1/review/${caseId}`).set(auth).send({ verdict: 'URGENCY_TOO_LOW', suggestedUrgency: 'urgent' });
    expect(again.status).toBe(409);

    const stats = await request(app).get('/v1/review/stats').set(auth);
    expect(stats.body).toMatchObject({ total: 1, agreementRate: 1, byVerdict: { AGREE: 1 } });
    // The caveat travels with the number.
    expect(stats.body.samplingNote).toMatch(/over-samples emergencies/i);

    // And the reviewed case leaves that reviewer's queue.
    const after = await request(app).get('/v1/review/queue').set(auth);
    expect(after.body.cases.map((x: { id: string }) => x.id)).not.toContain(caseId);
  });

  it('requires a suggested urgency for an urgency error, and rejects it otherwise', async () => {
    const { app, auth } = await reviewSetup();
    const { body: queue } = await request(app).get('/v1/review/queue').set(auth);
    const caseId = queue.cases[0].id;

    expect((await request(app).post(`/v1/review/${caseId}`).set(auth).send({ verdict: 'URGENCY_TOO_LOW' })).status).toBe(400);
    expect((await request(app).post(`/v1/review/${caseId}`).set(auth).send({ verdict: 'AGREE', suggestedUrgency: 'urgent' })).status).toBe(400);
    expect(
      (await request(app).post(`/v1/review/${caseId}`).set(auth).send({ verdict: 'URGENCY_TOO_LOW', suggestedUrgency: 'emergency' })).status,
    ).toBe(201);
  });

  it('snapshots the decision server-side, so a client cannot write its own evidence', async () => {
    const { app, auth, store } = await reviewSetup();
    const { body: queue } = await request(app).get('/v1/review/queue').set(auth);
    const caseId = queue.cases[0].id;
    const real = await store.reviews.caseById(caseId);

    await request(app)
      .post(`/v1/review/${caseId}`)
      .set(auth)
      // Fields a caller might hope to influence. None of them are read.
      .send({ verdict: 'WRONG_SPECIALTY', note: 'wrong department', urgency: 'self_care', rationale: ['invented.rule'], snapshot: { urgency: 'self_care' } });

    const stats = await store.reviews.stats();
    expect(stats.byVerdict.WRONG_SPECIALTY).toBe(1);
    // The contested rules come from the stored decision, not from the request body.
    expect(stats.contestedRules.map((r) => r.ruleId)).not.toContain('invented.rule');
    expect(stats.contestedRules.map((r) => r.ruleId)).toEqual(expect.arrayContaining([real!.rationale[0]!]));
  });

  /*
   * A review is evidence about a rule, not about a person, so it has to outlive the
   * retention window. If it did not, the safety record could never be longer than
   * seven days — which is the opposite of what a safety record is for.
   */
  it('keeps the review after the conversation it judged has been purged', async () => {
    const { app, auth, store } = await reviewSetup();
    const { body: queue } = await request(app).get('/v1/review/queue').set(auth);
    const caseId = queue.cases[0].id;
    await request(app).post(`/v1/review/${caseId}`).set(auth).send({ verdict: 'URGENCY_TOO_HIGH', suggestedUrgency: 'routine' });

    // Far enough into the future that every conversation is past its expiry.
    const purged = await store.conversations.purgeExpired(new Date(Date.now() + 999 * 86_400_000));
    expect(purged).toBeGreaterThan(0);

    const stats = await store.reviews.stats();
    expect(stats.total).toBe(1);
    expect(stats.byVerdict.URGENCY_TOO_HIGH).toBe(1);
    // The decision is gone, so there is nothing left to review and nothing to link to.
    expect(await store.reviews.caseById(caseId)).toBeNull();
    expect(stats.pending).toBe(0);
  });

  it('lets an administrator review, but never the reverse', async () => {
    const adminKey = 'a-very-long-operator-access-key-123';
    const { app } = await reviewSetup({ ADMIN_ACCESS_KEY: adminKey });

    const admin = await request(app).post('/v1/auth/admin').send({ accessKey: adminKey });
    const adminAuth = { Authorization: `Bearer ${admin.body.token}` };
    expect((await request(app).get('/v1/review/queue').set(adminAuth)).status).toBe(200);

    const reviewer = await request(app).post('/v1/auth/reviewer').send({ accessKey: REVIEWER_KEY });
    const reviewerAuth = { Authorization: `Bearer ${reviewer.body.token}` };
    expect((await request(app).get('/v1/admin/metrics').set(reviewerAuth)).status).toBe(403);
  });
});
