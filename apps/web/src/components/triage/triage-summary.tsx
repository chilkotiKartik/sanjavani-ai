...
import { CheckInOffer } from '@/components/checkins/checkin-offer';
...
export function TriageSummary({
  ...
  /** Used only to avoid offering a second check-in for a conversation that has one. */
  ...
  return (
    <Stagger className="space-y-6">
        ...
        <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <dd className="font-semibold text-[var(--ink)]">
              {triage.subject && triage.subject !== 'unknown'
                ? `${t(`subject_${triage.subject}` as MessageKey)}${triage.ageGroup === 'unknown' ? '' : ` · ${t(`age_${triage.ageGroup}` as MessageKey)}`}`
                : t(`age_${triage.ageGroup}` as MessageKey)}
            </dd>
          </div>
        </dl>
      </StaggerItem>
      ...
      {/* Offered after the advice, never instead of it, and never for an emergency. */}
      <StaggerItem>
        <CheckInOffer triage={triage} conversationId={conversationId ?? null} />
      </StaggerItem>
