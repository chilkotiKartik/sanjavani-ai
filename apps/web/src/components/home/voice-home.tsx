...
import { CheckInPrompt } from '@/components/checkins/checkin-prompt';
...
const LANGUAGE_CYCLE: LanguagePreference[] = ['auto', 'hi', 'hinglish', 'bn', 'en'];
const SHORT_LANGUAGE: Record<LanguagePreference, string> = { auto: 'Auto', hi: 'हिं', hinglish: 'Hing', bn: 'বাং', en: 'EN' };
    ...
    <div className="relative flex min-h-dvh flex-col">
      ...
      <main id="main" className="relative z-10 mx-auto flex w-full max-w-3xl flex-1 flex-col items-center px-4">
          ...
          {agent.answeredOffline && (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                {/*
                  Shown only when a hospital list is actually on screen. It is the one
                  thing offline that could mislead: the rules are exact, the directory
                  is a snapshot, and the difference has to be visible.
                */}
                {latestResponse?.degraded.includes('facilities_offline') && (
                  <p className="mt-1.5 text-sm leading-relaxed font-medium text-[var(--u-urgent)]">{t('offlineHospitals')}</p>
                )}
              </div>
            </motion.div>
          )}
        ...
        <div className="w-full max-w-xl space-y-3 pb-3">
          ...
          {/*
            A due check-in sits above the triage card: it is the one thing on this
            screen the app asked for, rather than the person. Saying "worse" starts a
            fresh turn carrying that fact, so the rules re-run on it.
          */}
          <CheckInPrompt
            onWorse={(checkIn) => {
              const what = checkIn.symptoms.length > 0 ? `${checkIn.symptoms.join(', ')} — ` : '';
              void agent.send(`${what}${t('ciWorseHandover')}`, 'text');
            }}
          />
          ...
          <p className="text-center text-xs leading-relaxed text-[var(--ink-faint)]">
            ...
            {/*
              Only before the first word is spoken. Someone who has never seen this
              needs to know what it is; someone mid-conversation does not need another
              link competing with the answer they are waiting for.
            */}
            {isFresh && (
              <>
                {' · '}
      ...
      <Sheet open={summaryOpen && Boolean(triage)} onClose={() => setSummaryOpen(false)} title={t('summary')}>
        {triage && latestResponse && (
          <div className="space-y-4">
            <TriageSummary triage={triage} facilities={latestResponse.facilities} conversationId={agent.conversationId} />
