import { SPECIALTY_LABELS, SYMPTOM_LABELS, pick, type Localized } from '@sanjeevani/medical-safety';
...
export function symptomLabel(code: string, language: Language): string {
  ...
  return label ? pick(label, language) : code.replace(/_/g, ' ');
}

export function specialtyLabel(code: string, language: Language): string {
  const label = SPECIALTY_LABELS[code as Specialty];
  return label ? pick(label, language) : code.replace(/_/g, ' ');
}
...
const RATIONALE: Record<string, Localized> = {
 ...
 */
const RULED_OUT: Record<string, Localized> = {
...
export function ruledOutText(screenId: string, language: Language): string | null {
  const t = RULED_OUT[screenId];
  return t ? pick(t, language) : null;
}

export function rationaleText(ruleId: string, language: Language): string | null {
  const known = RATIONALE[ruleId];
  if (known) return pick(known, language);
  if (ruleId.startsWith('severity.severe:')) {
    const s = symptomLabel(ruleId.split(':')[1] ?? '', language);
    return pick({ en: `You described the ${s} as severe.`, hi: `आपने ${s} को तेज़ बताया।`, hinglish: `Aapne ${s} ko tez bataya.` }, language);
  }
  if (ruleId.startsWith('screen.') && ruleId.endsWith('.positive')) {
    return pick({ en: 'You confirmed a warning sign.', hi: 'आपने एक खतरे का लक्षण बताया।', hinglish: 'Aapne ek khatre ka lakshan bataya.' }, language);
  }
  if (ruleId.startsWith('base:')) {
    ...
    return pick({ en: `${capitalize(s)} usually needs a doctor’s review.`, hi: `${s} में आमतौर पर डॉक्टर की सलाह चाहिए।`, hinglish: `${capitalize(s)} mein aam taur par doctor ki salah chahiye.` }, language);
  }
...
export function durationLabel(hours: number | null | undefined, language: Language): string | null {
  ...
  if (hours < 24) {
    ...
    return pick({ en: `${h} hour${h === 1 ? '' : 's'}`, hi: `${h} घंटे`, hinglish: `${h} ghante`, bn: `${h} ঘণ্টা` }, language);
  }
  const d = Math.round(hours / 24);
  if (d < 14) return pick({ en: `${d} day${d === 1 ? '' : 's'}`, hi: `${d} दिन`, hinglish: `${d} din`, bn: `${d} দিন` }, language);
  const w = Math.round(d / 7);
  return pick({ en: `${w} weeks`, hi: `${w} हफ्ते`, hinglish: `${w} hafte`, bn: `${w} সপ্তাহ` }, language);
}
