...
/**
 * The clinician review console talks to the API with a bearer token rather than the
 * session cookie every other call uses.
 *
 * Two reasons. The cookie identifies the anonymous person using the app, and a
 * reviewer is not that person — reusing it would conflate a patient session with a
 * clinical one on the same device. And a bearer header is not sent automatically by
 * the browser, so the review surface has no CSRF exposure at all rather than a
 * mitigated one.
 *
 * The token is never written to a log, a query string or a URL; it appears only in an
 * Authorization header, and `useReviewer` keeps it in sessionStorage so it dies with
 * the tab.
 */
const reviewRequest = <T,>(path: string, token: string, init: RequestInit = {}): Promise<T> =>
  request<T>(path, { ...init, headers: { ...(init.headers ?? {}), Authorization: `Bearer ${token}` } }, false);

export const reviewApi = {
  signIn: (accessKey: string) =>
    request<{ token: string; role: string; expiresAt: string }>('/v1/auth/reviewer', json({ accessKey }), false),
  queue: (token: string, limit = 10) => reviewRequest<ReviewQueueResponse>(`/v1/review/queue?limit=${limit}`, token),
  submit: (token: string, id: string, body: SubmitReviewRequest) =>
    reviewRequest<{ recorded: true }>(`/v1/review/${id}`, token, json(body)),
  stats: (token: string) => reviewRequest<ReviewStats & { samplingNote: string }>('/v1/review/stats', token),
};
