...
const GREETING: Record<Language, string> = {
  ...
  bn: 'নমস্কার, আমি সঞ্জীবনী। বলুন কী কষ্ট হচ্ছে — বাংলা, হিন্দি বা ইংরেজি, যেভাবে আপনার সুবিধা।',
};
        ...
        if (transportFailure) {
          try {
            ...
            const offline = await offlineTurn({
              ...
              languagePreference: prefs.language,
              ...(sttLanguage ? { sttLanguage } : {}),
              // Location is passed offline too: the bundled directory is ranked on
              // this device, so it never leaves it, and without a position there is
