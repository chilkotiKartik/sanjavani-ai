...
/**
 * hi-IN recognises Hindi, Hinglish and most English words, so it's the best default
 * for "auto". Bengali is the exception: hi-IN does not transcribe it, so an explicit
 * Bengali preference switches the recogniser. "auto" stays on hi-IN because the
 * recogniser has to be told a language before the first word is spoken, and guessing
 * bn-IN for everyone would break the far commoner Hindi case; once a turn comes back
 * detected as Bengali, `last` carries it.
 */
export function recognitionLang(pref: LanguagePreference, last: Language): string {
  if (pref === 'bn') return 'bn-IN';
  if (pref === 'en') return 'en-IN';
  if (pref === 'auto' && last === 'bn') return 'bn-IN';
  return 'hi-IN';
}
