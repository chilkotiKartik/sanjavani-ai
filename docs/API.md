...
### `POST /v1/auth/reviewer`
Body: `{ "accessKey": "…" }`. Issues a REVIEWER session as a bearer token:
`{ token, role: "REVIEWER", expiresAt }`. Only mounted when `REVIEWER_ACCESS_KEY` is set
(minimum 24 characters); otherwise `404`, not `401` — an endpoint that is switched off
should not advertise that it exists. Both staff logins compare the key in constant time
and audit a failure with a salted hash of the caller's IP.

Roles are a floor, not an equality test: ADMIN satisfies a REVIEWER requirement, REVIEWER
does not satisfy an ADMIN one.
...
Every route here requires a REVIEWER (or ADMIN) session and returns `404` when
`REVIEWER_ACCESS_KEY` is unset. See **Clinician review** in `docs/SAFETY.md` for what
this is for and why it shows so little.

### `GET /v1/review/queue` — **auth, REVIEWER**
Query: `limit` (1–25, default 10). Returns
`{ cases: ReviewCase[], remaining: number }` — decisions this reviewer has not yet
judged, ordered by consequence (emergencies, then low confidence, then newest).

A `ReviewCase` is a closed allow-list:
`{ id, decidedAt, language, urgency, emergency, emergencyCategory, specialty, confidence, source, ageGroup, durationHours, symptoms: [{ code, severity }], rationale: string[] }`.

There is no message text, conversation id, user id or location in it, the query never
reads the conversations table, and the response is re-validated against the schema on
the way out so a new column cannot ride along.

### `POST /v1/review/:id` — **auth, REVIEWER**
Body: `{ verdict, suggestedUrgency?, note? }`.
`verdict` is one of `AGREE`, `URGENCY_TOO_LOW`, `URGENCY_TOO_HIGH`, `WRONG_SPECIALTY`,
`INSUFFICIENT`. `suggestedUrgency` is **required** for the two urgency verdicts and
rejected for the others. `note` is free text, max 500 characters, sanitised before
storage.

`201` with `{ recorded: true }`. `409` if this reviewer has already judged this decision
— a second opinion from the same person is not a correction, and overwriting would lose
a judgement. `404` if the decision has since been purged.

The snapshot stored with the review is read server-side from the decision; nothing in
the request body contributes to it.

### `GET /v1/review/stats` — **auth, REVIEWER**
`{ total, agreementRate, byVerdict, contestedRules: [{ ruleId, disagreements }], pending, samplingNote, disagreementVerdicts }`.

`agreementRate` is `null` until the first review. `samplingNote` ships with the number
and says what it is: the queue is ordered by consequence, so the rate is a floor
measured on the hardest cases rather than a population estimate.
