...
| `REVIEWER_ACCESS_KEY` | Optional, ≥ 24 chars. Enables the clinician review console at `/review` and the `/v1/review/*` routes. Kept separate from `ADMIN_ACCESS_KEY` so reviewers can be given access without the operator surface, and either can be rotated alone. Unset, both return `404` |
