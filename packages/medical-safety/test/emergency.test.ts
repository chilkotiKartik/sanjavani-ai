...
const POSITIVE: [string, EmergencyCategory][] = [
  ...
  // Bengali (Bangla script), and the romanised spellings STT returns for Bengali speech
  ['বাবার বুকে ব্যথা হচ্ছে আর ঘাম হচ্ছে', 'cardiac'],
  ['শ্বাস নিতে পারছে না', 'breathing'],
  ['ঠোঁট নীল হয়ে যাচ্ছে', 'breathing'],
  ['মুখ বেঁকে গেছে আর কথা জড়িয়ে যাচ্ছে', 'stroke'],
  ['দাদু অজ্ঞান হয়ে গেছে', 'unconscious'],
  ['ছাদ থেকে পড়ে গেছে', 'major_trauma'],
  ['রক্ত বন্ধ হচ্ছে না', 'severe_bleeding'],
  ['খিঁচুনি হচ্ছে', 'seizure'],
  ['গলা ফুলে যাচ্ছে আর শ্বাস নিতে কষ্ট', 'anaphylaxis'],
  ['ও বিষ খেয়েছে', 'poisoning'],
  ['সাপে কেটেছে', 'poisoning'],
  ['আর বাঁচতে ইচ্ছে করছে না', 'self_harm'],
  ['আগুন লেগে গেছে, খুব পুড়ে গেছে', 'severe_burn'],
  ['পেটে বাচ্চা নড়ছে না', 'obstetric'],
  ['বাচ্চা দুধ খাচ্ছে না', 'infant_danger'],
  ['অ্যাম্বুলেন্স দরকার', 'user_requested'],
  ['amar baba ogyan hoye geche', 'unconscious'],
  ['shape kateche', 'poisoning'],
];
...
describe('emergency circuit breaker', () => {
  ...
  it.each([
    ...
    'আমার সামান্য সর্দি আর কাশি হয়েছে',
    'তিন দিন ধরে জ্বর, আর কিছু নয়',
    'আমার বুকে ব্যথা নেই, শুধু কাশি',
    'গত বছর আমার বুকে ব্যথা হয়েছিল, এখন ঠিক আছি',
    'যদি কারো বুকে ব্যথা হয় তাহলে কী করা উচিত',
  ])('does not trigger for "%s"', (text) => {
    expect(assess(text).emergency).toBe(false);
  });

  it('reads Bengali past and hypothetical framing', () => {
    expect(detectFraming('গত বছর আমার বুকে ব্যথা হয়েছিল')).toBe('past');
    expect(detectFraming('যদি কারো বুকে ব্যথা হয় তাহলে কী করব')).toBe('hypothetical');
    // A Bengali present-time assertion overrides the past marker, as in every language.
    expect(detectFraming('গত বছরও হয়েছিল, এখনই আবার হচ্ছে')).toBe('present');
  });

  it('still fires on an explicit Bengali emergency phrase, whatever the framing', () => {
    // Phrase rules are never suppressed — someone typing "snake bite" is reporting one.
    expect(assess('যদি সাপে কাটে তাহলে কী করব').emergency).toBe(true);
  });
