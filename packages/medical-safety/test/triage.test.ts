...
/*
 * A turn that describes something finished must not become a present complaint.
 *
 * The circuit breaker already held its composite rules back on past framing, so these
 * cases never produced an emergency takeover — and that was enough to make them look
 * fine. The symptom still landed in clinical state, and triage answered "Call 112 now
 * or go to the nearest emergency department". The tests below pin down the part that
 * was actually broken, and the veto that must keep working.
 */
describe('finished episodes', () => {
  it('records no present symptom from an explicitly past turn', () => {
    const state = applyUserInput(emptyClinicalState(), 'I had chest pain last year but it was checked and I am fine now').state;
    expect(state.symptoms).toHaveLength(0);
    expect(evaluateTriage(state).urgency).not.toBe('emergency');
  });

  it('does the same in Hinglish, where the time marker trails the symptom', () => {
    const state = applyUserInput(emptyClinicalState(), 'Pichle saal seene mein dard hua tha, ab theek hoon').state;
    expect(state.symptoms).toHaveLength(0);
  });

  it('is vetoed the moment anything says it is happening now', () => {
    const state = applyUserInput(emptyClinicalState(), 'I had chest pain last year and it is happening again right now').state;
    expect(state.symptoms.some((s) => s.code === 'chest_pain')).toBe(true);
    expect(evaluateTriage(state).urgency).toBe('emergency');
  });

  it('cannot retract a symptom that is already known', () => {
    // The dangerous direction: a later framing word must not unwind an escalation
    // the person has already been shown.
    const present = applyUserInput(emptyClinicalState(), 'I have severe chest pain').state;
    expect(present.symptoms.some((s) => s.code === 'chest_pain')).toBe(true);
    const after = applyUserInput(present, 'that also happened last year').state;
    expect(after.symptoms.some((s) => s.code === 'chest_pain')).toBe(true);
    expect(evaluateTriage(after).urgency).toBe('emergency');
  });

  it('still records a negation on an ordinary turn', () => {
    // The suppression is specific to past framing, not a general switching-off of
    // extraction: an unframed sentence behaves exactly as before.
    const state = applyUserInput(emptyClinicalState(), 'I have a headache but no chest pain').state;
    expect(state.negatedSymptoms).toContain('chest_pain');
    expect(state.symptoms.some((s) => s.code === 'headache')).toBe(true);
  });
});
 ...
 * Age-aware triage.
 *
 * The clinical point of the age layer is that the same symptom means different things
 * in different bodies. A fever in a two-month-old is not the same event as a fever in
 * a thirty-year-old, and an app that treated them alike would be wrong in the
 * direction that costs the most.
 */
describe('who the advice is for', () => {
  it('escalates an infant that an adult would not be escalated for', () => {
    const adult = converse(['I have had a fever since yesterday', 'no']);
    const infant = converse(['my 2 month old baby has a fever', 'no']);
    expect(infant.ageGroup).toBe('infant');
    expect(urgencyRank(evaluateTriage(infant).urgency)).toBeGreaterThan(urgencyRank(evaluateTriage(adult).urgency));
  });

  it('carries the relationship onto the result, not just the age band', () => {
    const state = applyUserInput(emptyClinicalState(), 'mere bete ko bukhar hai').state;
    expect(state.subject).toBe('child');
    const result = buildTriageResult(evaluateTriage(state), state, 'en', [], 'rules');
    expect(result.subject).toBe('child');
  });

  it('narrows a vague subject but never widens a specific one', () => {
    // "Someone is unwell… it's my son" should narrow; the reverse must not happen.
    let state = applyUserInput(emptyClinicalState(), 'he has a fever').state;
    expect(state.subject).toBe('other');
    state = applyUserInput(state, 'it is my son').state;
    expect(state.subject).toBe('child');
    state = applyUserInput(state, 'he is still unwell').state;
    expect(state.subject).toBe('child');
  });

  it('gives an adolescent the fluid advice written for children', () => {
    // The one place the band changes behaviour rather than just labelling: a
    // fifteen-year-old with vomiting dehydrates the way a child does.
    const teen = converse(['my 15 year old son has been vomiting since morning', 'no']);
    expect(teen.ageGroup).toBe('adolescent');
    expect(evaluateTriage(teen).care).toContain('child_fluids');
  });

  it('routes an adolescent to an adult department, deliberately', () => {
    // Sending a sixteen-year-old to a paediatric department that may not accept them
    // is a worse failure than a coarse label, so specialty routing treats them as
    // adults even though fluid advice does not.
    const teen = converse(['my 16 year old has a bad cough', 'no']);
    const child = converse(['my 6 year old has a bad cough', 'no']);
    expect(evaluateTriage(child).specialty).toBe('pediatrics');
    expect(evaluateTriage(teen).specialty).not.toBe('pediatrics');
  });

  it('asks who it is for when someone else is described without an age', () => {
    const state = applyUserInput(emptyClinicalState(), 'my father has a bad headache').state;
    expect(state.subject).toBe('parent');
    expect(state.ageGroup).toBe('unknown');
    // Age genuinely changes the answer here, so asking is warranted rather than noise.
    const slot = planFollowUp(state);
