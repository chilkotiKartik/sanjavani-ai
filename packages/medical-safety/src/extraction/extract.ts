import type { AgeGroup, Duration, ExtractedSymptom, Severity, Subject, SymptomCode } from '@sanjeevani/types';
...
const UNIT_WORDS: Record<string, Unit> = {
  ...
  ghonta: 'hour', 'ঘণ্টা': 'hour', 'ঘন্টা': 'hour',
  'দিন': 'day',
  saptaho: 'week', 'সপ্তাহ': 'week', 'হপ্তা': 'week',
  mash: 'month', 'মাস': 'month',
  bochor: 'year', bochhor: 'year', 'বছর': 'year',
};
/** Words that introduce an age: "umar 40", "boyos 6 mash". */
const AGE_WORDS = new Set(['age', 'aged', 'umar', 'umr', 'उम्र', 'आयु', 'boyos', 'বয়স', 'বয়সী']);
/** Words that mark the *preceding* number as an age: "2 saal ka", "6 mash boyos". */
const AGE_MARKERS = new Set(['old', 'ka', 'ki', 'ke', 'का', 'की', 'के', 'aged', 'boyos', 'বয়স', 'বয়সী']);
const DURATION_BEFORE = new Set(['for', 'since', 'past', 'last', 'pichle', 'pichhle', 'पिछले', 'from', 'গত', 'গতো', 'goto']);
const DURATION_AFTER = new Set(['se', 'से', 'from', 'ho', 'hue', 'hua', 'now', 'ago', 'pehle', 'पहले', 'hogaye', 'gaye', 'tak', 'ধরে', 'আগে', 'যাবৎ', 'dhore']);
...
const RELATIVE_DURATIONS: { phrase: CompiledPhrase; hours: number; label: string }[] = [
  ...
  ['এখনই', 0.5], ['একটু আগে', 0.5], ['ektu age', 0.5],
  ['আজ সকাল থেকে', 6], ['সকাল থেকে', 6], ['আজ থেকে', 6], ['sokal theke', 6],
  ['কাল রাত থেকে', 12], ['রাত থেকে', 12], ['kal rat theke', 12],
  ['কাল থেকে', 24], ['গতকাল থেকে', 24], ['kal theke', 24],
  ['পরশু থেকে', 48], ['porshu theke', 48],
  ['গত সপ্তাহ থেকে', 168], ['গত সপ্তাহে', 168],
  ['গত মাস থেকে', 720], ['গত মাসে', 720],
...
const INFANT_WORDS = compileAll(
  ['newborn', 'new born', 'infant', 'navjaat', 'shishu', 'नवजात', 'शिशु', 'nobojatok', 'নবজাতক', 'সদ্যোজাত'],
  0,
);
const CHILD_WORDS = compileAll(
  [
    ...
    'baccha', 'chele', 'meye', 'khoka', 'khuki',
    'বাচ্চা', 'বাচ্চার', 'শিশু', 'ছেলে', 'ছেলের', 'মেয়ে', 'মেয়ের', 'খোকা', 'খুকি',
  ],
  0,
);
const OLDER_WORDS = compileAll(
  [
    ...
    'thakurda', 'thakuma', 'dadu', 'dida', 'briddho', 'boyosko',
    'ঠাকুরদা', 'ঠাকুমা', 'দাদু', 'দিদা', 'বৃদ্ধ', 'বৃদ্ধা', 'বয়স্ক',
  ],
  0,
);
const SELF_WORDS = new Set([
  ...
  'ami', 'amar', 'amake', 'amay', 'nijer',
  'আমি', 'আমার', 'আমাকে', 'আমায়', 'নিজের',
]);
const OTHER_WORDS = new Set([
  ...
  'take', 'tar', 'tader', 'shami', 'stri', 'bondhu', 'kaku', 'kakima', 'mashi', 'jethu',
  'তাকে', 'তার', 'তাদের', 'স্বামী', 'স্ত্রী', 'বন্ধু', 'কাকু', 'কাকিমা', 'মাসি', 'জেঠু',
]);

/*
 * Relationship words, which say *who* without saying how old.
 *
 * The distinction matters. "My son" tells the app that asking an age is worthwhile;
 * it does not tell it the son is six rather than thirty-six, and a system that assumed
 * otherwise would quietly apply paediatric thresholds to an adult. So these set the
 * subject and nothing else — the age band still comes from an explicit age, an
 * unambiguous word like "newborn", or the question.
 */
const PARENT_WORDS = compileAll(
  [
    'father', 'mother', 'dad', 'daddy', 'mom', 'mum', 'papa', 'pappa', 'mummy', 'mama',
    'pitaji', 'pita ji', 'pita', 'mataji', 'mata ji', 'abbu', 'ammi', 'walid', 'walida',
    'पापा', 'मम्मी', 'पिता', 'पिताजी', 'माता', 'माताजी', 'माँ', 'मां', 'अब्बू', 'अम्मी',
    'baba', 'babar', 'maa', 'mayer', 'bapi',
    'বাবা', 'বাবার', 'মা', 'মায়ের', 'বাপি', 'আব্বা', 'আম্মা',
  ],
  0,
);
const OFFSPRING_WORDS = compileAll(
  [
    // Phonetic, not literal, so the oblique forms people actually say are covered:
    // "mere bete ko", "bachche ko", "beti ko" all reach the same key as the root.
    'son', 'daughter', 'child', 'kid', 'beta', 'beti', 'bete', 'bachcha', 'bacha', 'bachche', 'bacche', 'bachchi', 'bachi',
    'बेटा', 'बेटी', 'बेटे', 'बच्चा', 'बच्चे', 'बच्ची',
    // Bengali marks the possessor, not the child, so both forms are listed:
    // "amar cheler jor" and "chele" reach the same subject.
    'chele', 'cheler', 'meye', 'meyer', 'baccha', 'bacchar',
    'ছেলে', 'ছেলের', 'মেয়ে', 'মেয়ের', 'বাচ্চা', 'বাচ্চার', 'সন্তান',
  ],
  0,
);

/*
 * Bands, from a stated age.
 *
 * The boundaries are conventional rather than clinical — under one, under thirteen,
 * under eighteen, sixty and over. They are engineering defaults chosen so that nobody
 * lands in a band whose advice is written for a different body, and every one of them
 * errs towards the more cautious side of the line: a twelve-year-old is a child, and a
 * sixty-year-old is an older adult.
 */
...
export function extractAge(text: string): AgeExtraction {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (AGE_WORDS.has(raw[i]!)) {
      // "umar 6 mahine" and "boyos chhoy mash" are six *months*, not six years. The
      // number after an age word was being taken as years whatever unit followed it,
      // which quietly aged an infant into a six-year-old and switched off every
      // infant rule in the emergency layer. Read the unit when one is there.
      const offset = [1, 2, 3].find((k) => parseNumber(raw[i + k]) !== null);
      const next = offset === undefined ? null : parseNumber(raw[i + offset]!);
      if (next !== null && next < 120) {
        const unitAfter = offset === undefined ? undefined : UNIT_WORDS[raw[i + offset + 1] ?? ''];
        years =
          unitAfter === 'month' ? next / 12
          : unitAfter === 'week' ? (next * 7) / 365
          : unitAfter === 'day' ? next / 365
          : next;
      }
    }
  }
  ...
  const hasParent = findAny(prepared, PARENT_WORDS);
  const hasOffspring = findAny(prepared, OFFSPRING_WORDS);
  const hasOther = raw.some((t) => OTHER_WORDS.has(t));

  /*
   * Someone else always wins over "self".
   *
   * "Mere bete ko bukhar hai" contains *mere*, a self word, and is plainly about the
   * son. Reading the possessive as the patient would apply the wrong age thresholds to
   * a child, which is the dangerous direction to be wrong in.
   */
  const subject: Subject = hasOffspring ? 'child' : hasParent ? 'parent' : hasOther ? 'other' : hasSelf ? 'self' : 'unknown';

  if (years !== null) return { ageGroup: ageGroupFromYears(years), years, subject };

  // Only words that are unambiguous about age set a band. A relationship never does.
  if (findAny(prepared, INFANT_WORDS)) return { ageGroup: 'infant', years: null, subject: subject === 'unknown' ? 'child' : subject };
  if (findAny(prepared, CHILD_WORDS)) return { ageGroup: 'child', years: null, subject: subject === 'unknown' ? 'child' : subject };
  if (findAny(prepared, OLDER_WORDS)) return { ageGroup: 'older_adult', years: null, subject: subject === 'unknown' ? 'other' : subject };
  if (subject === 'self') return { ageGroup: 'adult', years: null, subject };
  return { ageGroup: 'unknown', years: null, subject };
}
...
const YES = new Set(
  [
    'yes', 'yeah', 'yep', 'yup', 'haan', 'han', 'ha', 'haa', 'hanji', 'ji', 'bilkul', 'sahi', 'correct', 'right',
    'हाँ', 'हां', 'जी', 'बिल्कुल', 'हाँजी',
    'hyan', 'hyaan', 'hae', 'thik', 'ekdom',
    'হ্যাঁ', 'হ্যা', 'হ্যাঁ গো', 'ঠিক', 'একদম', 'আছে',
  ].map(phoneticKey),
);
const NO = new Set(
  [
    'no', 'nope', 'nahi', 'nahin', 'nai', 'na', 'nhi', 'नहीं', 'नही', 'ना', 'not',
    'nei', 'noy',
    'না', 'নেই', 'নাই', 'নয়', 'একদম না',
  ].map(phoneticKey),
);
...
export function parseSeverityAnswer(text: string): Severity | null {
  ...
  if (findAny(prepared, compileAll(['theek', 'thik', 'normal', 'manageable', 'bearable', 'ठीक', 'ঠিক আছে', 'সহ্য করা যায়'], 0))) return 'mild';
  return null;
}
