...
describe('conversation engine (demo providers)', () => {
  ...
  /*
   * Bengali end to end, because the language is only real if the whole turn is in it:
   * the detected language, the acknowledgement, the follow-up question, the care
   * advice, the warning signs and the disclaimer. The assertions below check the
   * pieces written by different layers — the question comes from the safety package,
   * the acknowledgement from the response templates, the disclaimer from guidance —
   * so an untranslated layer fails here rather than reaching someone as English.
   */
  it('runs a full Bengali journey and answers in Bengali at every layer', async () => {
    const engine = deterministic();
    const { responses } = await run(engine, ['আমার দু দিন ধরে খুব জ্বর আর গা ব্যথা', 'না']);
    const [first, second] = responses.map((r) => r.response);

    expect(first!.language).toBe('bn');
    expect(first!.phase).toBe('follow_up');
    expect(first!.reply.display).toMatch(/বুঝলাম/);
    expect(first!.quickReplies.map((q) => q.label)).toEqual(['হ্যাঁ', 'না']);

    expect(second!.phase).toBe('advice');
    expect(second!.triage?.urgency).toBe('routine');
    expect(second!.triage?.recommendedAction).toMatch(/ওপিডি/);
    expect(second!.triage?.careAdvice.join(' ')).toMatch(/বিশ্রাম/);
    expect(second!.triage?.warningSigns.join(' ')).toMatch(/শ্বাস/);
    expect(second!.disclaimer).toMatch(/সঞ্জীবনী/);
    // Nothing in the reply should have fallen back to Hindi. The range is Devanagari
    // *letters* only: the danda (U+0964) that ends these sentences lives in the
    // Devanagari block but is the shared South Asian full stop, and Bengali uses it.
    expect(second!.reply.display).not.toMatch(/[\u0904-\u0939]/);
  });

  it('gives a Bengali emergency its instructions in Bengali', async () => {
    const engine = deterministic();
    const { last } = await run(engine, ['বাবার বুকে খুব ব্যথা হচ্ছে আর ঘাম হচ্ছে']);
    const r = last.response;
    expect(r.language).toBe('bn');
    expect(r.emergency?.category).toBe('cardiac');
    expect(r.emergency?.headline).toMatch(/হৃদরোগ/);
    expect(r.emergency?.instructions[0]).toMatch(/১১২/);
    expect(r.emergency?.contacts.some((c) => c.number === '112')).toBe(true);
  });
