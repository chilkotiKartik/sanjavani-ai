...
export class ConversationEngine {
  ...
  private async runTurn(previous: ConversationMemory, input: TurnInput): Promise<TurnOutcome> {
    ...
    switch (intent) {
      case 'reset': {
        ...
        return this.finish(fresh, 'greeting', 'reset', language, pick(T.reset, language), [], 'not_needed', null, pickOf(STARTER_REPLIES, language), degraded, telemetry, tracer, started);
      }
      case 'greeting':
        if (allSymptoms(clinical).length === 0) {
          return this.finish(memory, 'greeting', 'greeting', language, pick(T.greeting, language), [], 'not_needed', null, pickOf(STARTER_REPLIES, language), degraded, telemetry, tracer, started);
        }
        break;
      case 'thanks':
        return this.finish(memory, 'closing', 'thanks', language, pick(T.thanks, language), [], 'not_needed', memory.lastTriage, [], degraded, telemetry, tracer, started);
      case 'directions':
      ...
      case 'add_symptom': {
        ...
        return this.finish(memory, 'follow_up', 'symptom_report', language, pick(T.addSymptom, language), [], 'not_needed', memory.lastTriage, prompt.quickReplies, degraded, telemetry, tracer, started);
      }
      default:
        break;
    }
    ...
    if (slot) {
      ...
      if (slot.kind === 'symptom_detail') {
        const text = intent === 'unrelated' && previous.turnCount === 0 ? pick(T.greeting, language) : pick(T.noSymptomsYet, language);
    ...
    if (allSymptoms(clinical).length === 0) {
      return this.finish(memory, 'clarify', 'unrelated', language, `${pick(T.clarify, language)}`, [], 'not_needed', memory.lastTriage, followUpPrompt({ kind: 'symptom_detail' }, language).quickReplies, degraded, telemetry, tracer, started);
    }
  ...
  private async emergencyTurn(
    ...
    return {
      response: {
        ...
        quickReplies: [],
        degraded: [...degraded],
        disclaimer: pick(DISCLAIMER.short, language),
  ...
  private async adviceTurn(
    ...
    const facts: PhrasingFacts = {
      ...
      recommendedAction:
        decision.urgency === 'emergency' || decision.leadSymptom === 'animal_bite'
          ...
          : pick(SPOKEN_ACTION[decision.urgency], language),
      careTips: decision.care.slice(0, 1).map((k) => pick(CARE_ADVICE[k], language)),
      facility: facilitySentence(top, status, decision.urgency, language),
      closingQuestion: top ? pick(T.offerDirections, language) : null,
    };
    ...
    return this.finish(memory, 'advice', intent, language, phrased.text, facilities, status, triage, pickOf(ADVICE_REPLIES, language), degraded, telemetry, tracer, started, {
      triageEvent: triage,
    });
  }
  ...
  private async facilitySearchTurn(
    ...
    if (!input.location) {
      return this.finish(memory, 'facility_search', 'find_facility', language, pick(T.needLocation, language), [], 'needs_location', memory.lastTriage, [], degraded, telemetry, tracer, started);
    }
 ...
 const text = top
      ? fill(pick(T.facilityList, language), { name: top.name, km: formatKm(top.travel.distanceMeters) })
      : (facilitySentence(undefined, result.status, 'routine', language) ?? pick(T.noneFound, language));
    if (result.facilities.length) memory.lastFacilities = result.facilities;
    return this.finish(memory, 'facility_search', 'find_facility', language, text, result.facilities, result.status, triage, top ? pickOf(ADVICE_REPLIES, language).slice(0, 1) : [], degraded, telemetry, tracer, started);
  }
  ...
  private async directionsTurn(
    ...
    if (!top) {
      ...
      return this.finish(memory, 'facility_search', 'directions', language, input.location ? pick(T.directionsNoFacility, language) : pick(T.needLocation, language), [], status, memory.lastTriage, [], degraded, telemetry, tracer, started);
    }
