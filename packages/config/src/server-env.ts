...
const envSchema = z
  .object({
    ...
    /**
     * Enables POST /v1/auth/reviewer and the clinician review console. Min 24 chars.
     * Separate from ADMIN_ACCESS_KEY so a clinic can hand reviewers a key without
     * handing them the operator surface, and can rotate one without the other. Unset,
     * the review routes return 404 rather than 401 — an endpoint that is switched off
     * should not advertise that it exists.
     */
    REVIEWER_ACCESS_KEY: optionalSecret,
    /**
